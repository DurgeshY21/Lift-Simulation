document.addEventListener('DOMContentLoaded', () => {
  ...
  const state = {
    floors: Array.from({ length: numberOfFloors }, (_, i) => ({ id: i, requests: [] })),
    lifts: Array.from({ length: numberOfLifts }, (_, i) => ({
      id: i,
      currentFloor: 0, // All lifts start at the 0th floor
      moving: false,
      doorsOpen: false,
      targetFloor: null, // Track the target floor for each lift
    })),
  };
  ...
  const floorWidth = totalLiftsWidth + 100; // Add extra space (e.g., 40px) for better appearance
  ...
  const requestLift = (floorId, direction) => {
    ...
    // Add request to the specific floor's request list
    state.floors[floorId].requests.push(direction);
  ...
  const processRequests = () => {
    state.floors.forEach(floor => {
      if (floor.requests.length === 0) return; // Skip floors without requests
      ...
      state.lifts.forEach(lift => {
        if (!lift.moving && !lift.doorsOpen && lift.targetFloor === null) { // Check if lift is not moving, doors closed, and no target
          const distance = Math.abs(lift.currentFloor - floor.id);
      ...
      if (closestLift) {
        ...
        moveLift(closestLift, floor.id);
        floor.requests.shift(); // Remove the request after assigning a lift
      }
    });
  };

  const moveLift = (lift, targetFloor) => {
    if (lift.moving || lift.doorsOpen || lift.targetFloor !== null) return; // Prevent moving if the lift is already busy
    ...
    setTimeout(() => {
      ...
      lift.targetFloor = null; // Reset target floor
